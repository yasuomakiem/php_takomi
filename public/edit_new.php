...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['action'])) {
    ...
    // Cập nhật DB
    $sql = "UPDATE news SET name = :name, image = :image, description = :description, category_id = :category_id WHERE id = :id";
    $stmt = $conn->prepare($sql);
    $stmt->execute([
        ':name' => $name,
        ':image' => $image,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_detail') {
    ...
    $sql = "INSERT INTO new_details (service_id, detail_name, detail_value, created_at, updated_at)
...
if (isset($_GET['delete_detail'])) {
    ...
    $sql = "DELETE FROM new_details WHERE id = :id AND service_id = :service_id";
    $stmt = $conn->prepare($sql);
